...
The example consists of the three modules template_model.py, which describes the system model, 
template_mpc.py, which defines the settings for the control 
and template_simulator.py, which sets the parameters for the simulator. 
...
# The dynamics of the double pendulum can be derived from 
# the Euler-Lagrangian equations, which yield:
...
"""
Energy equations
The next step is to introduce new “auxiliary” expressions to do-mpc for the kinetic 
and potential energy of the system. This is required in this example, 
because we will need these expressions for the formulation of the MPC controller.

Introducing these expressions has the additional advantage that do-mpc saves 
and exports the calculated values, which is great for monitoring and debugging.
...
# Objective
"""
The control objective is to errect the double pendulum 
and to stabilize it in the up-up position. 
It is not straight-forward to formulate an objective which yields this result. 
Classical set-point tracking, e.g. with the set-point: theta_s = [0,0,0]
and a quadratic cost:
is known to work very poorly.
Clearly, the problem results from the fact that 
theta_s = 2*pi*n, integer is also a valid solution.
"""
"""
Instead we will use an energy-based formulation for the objective. 
If we think of energy in terms of potential and kinetic energy 
it is clear that we want to maximize the potential energy (up-up position) 
...
# %% Simulator
"""
To create a simulator in order to run the MPC in a closed-loop, 
we create an instance of the do-mpc simulator which is based on the same model:
"""
